...
bool FrontendStereo::RejectOutliersBetweenRefernceLeftAndCurrentLeftImage() {
    ...
    for (uint32_t i = 0; i < cur_pixel_uv_left()->size(); ++i) {
        camera_models()[0]->LiftFromRawImagePlaneToUndistortedNormalizedPlane((*cur_pixel_uv_left())[i], (*cur_norm_xy_left())[i]);
    }
    ...
    if (epipolar_solver() == nullptr) {
        ...
        if (!feature_tracker()->TrackFeatures(*cur_pyramid_left(), *ref_pyramid_left(), *cur_pixel_uv_left(), ref_pixel_uv_left_tracked_back_,
                                              tracked_status())) {
...
bool FrontendStereo::TrackFeaturesFromCurrentLeftToCurrentRightImage(const GrayImage &cur_image_left, const GrayImage &cur_image_right) {
    ...
    *cur_pixel_uv_right() = *cur_pixel_uv_left();
    ...
    if (!feature_tracker()->TrackFeatures(*cur_pyramid_left(), *cur_pyramid_right(), *cur_pixel_uv_left(), *cur_pixel_uv_right(),
...
bool FrontendStereo::RejectOutliersBetweenCurrentLeftToCurrentRightImage() {
    ...
    for (uint32_t i = 0; i < cur_pixel_uv_right()->size(); ++i) {
        camera_models()[1]->LiftFromRawImagePlaneToUndistortedNormalizedPlane((*cur_pixel_uv_right())[i], (*cur_norm_xy_right())[i]);
    }
...
bool FrontendStereo::SupplememtNewFeatures(const GrayImage &cur_image_left) {
    // Detect new features.
    feature_detector()->DetectGoodFeatures(cur_image_left, options().kMaxStoredFeaturePointsNumber, *cur_pixel_uv_left());
    ...
    for (uint32_t i = 0; i < new_features_num; ++i) {
        ...
        camera_models()[0]->LiftFromRawImagePlaneToUndistortedNormalizedPlane((*cur_pixel_uv_left())[i + old_features_num], temp_cur_norm_xy_left);
...
bool FrontendStereo::RunOnce(const GrayImage &cur_image_left, const GrayImage &cur_image_right, const float time_stamp_s) {
    ...
    // If frontend is configured to select keyframe by itself, frontend will track features from fixed keyframe to
    // current frame.
...
void FrontendStereo::DrawTrackingResults(const std::string &title, const float time_stamp_s) {
    ...
    RgbImage show_image;
    Visualizor2D::DrawImageWithTrackedFeaturesWithId(ref_pyramid_left()->GetImage(0), ref_pyramid_right()->GetImage(0), cur_pyramid_left()->GetImage(0),
